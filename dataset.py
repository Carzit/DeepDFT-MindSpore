...
class RotatingPoolData(ms.dataset.Dataset):
    """
    ...
    def __init__(self, dataset, pool_size, **kwargs):
        ...
        self.data_pool = [
            self.parent_data[i] for i in self.rng.integers(0, high=len(self.parent_data), size=self.pool_size, endpoint=False).tolist()
        ]
...
class BufferData(ms.dataset.Dataset):
    """
...
class DensityData(ms.dataset.Dataset):
...
class DensityDataDir(ms.dataset.Dataset):
...
class DensityDataTar(ms.dataset.Dataset):
    ...
    def __next__(self):
        if self.current_slice < self.num_slices:
            ...
            while this_slice not in self.finished_slices:
                ...
                res = {k: ms.tensor(v) for k,v in res.items()} # convert to ms tensor.
...
def atoms_and_probe_sample_to_graph_dict(density, atoms, grid_pos, cutoff, num_probes):
    ...
    # used to be 'torch.get_default_dtype()' here.
    # mindspore seems not have similar impl/func/api
    # consider no set_default_dtype ops above/behind, i use ms.float32 here, in reference to torch.float32 in torch
...
def atoms_to_graph_dict(atoms, cutoff):
    ...
    # pylint: disable=E1102
    res = {"nodes": ms.tensor(atoms.get_atomic_numbers()),
           "atom_edges": ms.tensor(np.concatenate(atom_edges, axis=0)),
           "atom_edges_displacement": ms.tensor(np.concatenate(atom_edges_displacement, axis=0), dtype=default_type)
           }
    res["num_nodes"] = ms.tensor(res["nodes"].shape[0])
    res["num_atom_edges"] = ms.tensor(res["atom_edges"].shape[0])
    res["atom_xyz"] = ms.tensor(atoms.get_positions(), dtype=default_type)
